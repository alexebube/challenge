function * listProduct (initial, ...rest) {
  ...
  for (const r of remainder) for (const i of initial) yield [i, ...r]
}
...
const countPairs = (accumulator = {}) => (objectPairs = {}) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    accumulator[currentValue] = Object.prototype.hasOwnProperty.call(accumulator, currentValue) ? accumulator[currentValue] + 1 : 1
  }
  return accumulator
}
...
const groupUniquePairs = isEqual => (thresholdValue = 0) => (accumulator = {}) => (uniqueNamePair = {}) => (objectPairs = {}) => {
  ...
  for (let i = 0; i < len; i++) {
    ...
    for (let j = i + 1; j <= len; j++) {
      const b = keys[j]
      if (isEqual(a)(b)) {
        if (!Object.prototype.hasOwnProperty.call(accumulator, a)) {
    ...
    if (!Object.prototype.hasOwnProperty.call(accumulator, a)) {
      if (!Object.prototype.hasOwnProperty.call(uniqueNamePair, a)) {
